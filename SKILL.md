...
description: Run and supervise the scenario test defined by ScenarioTest.md. Use when asked to start, resume, monitor, diagnose, repair, or produce a repro package for ScenarioTest.md traffic tests. Performs a full preflight for each unvalidated machine/script/configuration combination, monitors adaptively, repairs script defects, uses targeted stale-state cleanup, and stops with a detailed manual repro package for non-script failures.
---
...
• Scenario batches: <completed>/48; <phase>, batch <index>, <stage>;
